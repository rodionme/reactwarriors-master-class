...
class LoginForm extends Component {
    ...
    this.setState(
      ...
      () => {
        if (Object.keys(this.state.errors).length === 0) {
          ...
          fetch(`https://api.themoviedb.org/3/authentication/token/new?api_key=${API_KEY_3}`)
            .then((response) => response.json())
            .then((data) => {
              const { request_token } = data;
              fetch(
                `https://api.themoviedb.org/3/authentication/token/validate_with_login?api_key=${API_KEY_3}&username=${this.state.username}&password=${this.state.password}
  ...
  getUser = (session_id) => {
    fetch(`https://api.themoviedb.org/3/account?api_key=${API_KEY_3}&session_id=${session_id}`)
      .then((response) => response.json())
      .then((data) => {
        this.setState({
          isLoading: false,
